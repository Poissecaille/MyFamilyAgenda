...
const timezoneConvert = (struct) => {
  let tmp = []
  let i = 0
  for (let timezone of struct) {
    for (let row of timezone['newRow']) {
      i++
      delete row["id"]
      row["id"] = i
      delete row["country"]
...
const getCurrentTZDateTime = (tzString) => {
  let date = new Date()
  let tzDate = new Date((typeof date === "string" ? new Date(date) : date).toLocaleString("en-US", { timeZone: tzString }));
  let formattedtzDate = new Date(tzDate.getTime() - tzDate.getTimezoneOffset() * 60 * 1000)
  return formattedtzDate
};

const getCurrentTZDateTimeString = (tzString) => {
  let date = new Date()
  let tzDate = new Date((typeof date === "string" ? new Date(date) : date).toLocaleString("en-US", { timeZone: tzString }));
  let formattedtzDate = new Date(tzDate.getTime() - tzDate.getTimezoneOffset() * 60 * 1000).toISOString()
  return formattedtzDate.split('T')[0] + " " + formattedtzDate.split('T')[1].split('.')[0]
};

export const timestampToTimeAndTimeString = (timestamp) => {
  let date = new Date(timestamp)
  return [date, date.toLocaleTimeString()];
};

export const actualDatetime = getCurrentTZDateTime("Europe/Paris")
export const actualDatetimeString = getCurrentTZDateTimeString("Europe/Paris")
export const timezoneStruct = timezoneConvert(timezones)

export const getDatesBetweenTwoDates = (firstDateString, secondDateString) => {
  let result = [];
  let dateMove = new Date(firstDateString)
  while (firstDateString < secondDateString) {
    firstDateString = dateMove.toISOString().slice(0, 10);
    result.push(firstDateString);
...
export const weatherApiStruct = {
  200: require('../assets/storm.png'),
  300: require('../assets/cloudy.png'),
  500: require('../assets/rain.png'),
  600: require('../assets/snow.png'),
  700: require('../assets/fog.png'),
  800: require('../assets/sun.png'),
  801: require('../assets/sun_and_clouds.png')
};

export const discordSounds = ['super_cute_02.wav', 'super_cute_04.wav', 'super_cute_06.wav', 'super_cute_09.wav']
